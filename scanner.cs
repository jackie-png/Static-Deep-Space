...
public class scanner : MonoBehaviour
{
    ...
    void Update()
    {
        if (controller != null) {
            ...
            if (primaryTrigger > threshold && canFire)
            {
    ...
    private void turnOffControllerModel() {
        ...
        OVRControllerHelper helper = controller.GetComponent<OVRControllerHelper>();

        helper.SetControllerVisibility(false);
        ...
        for (int row = 0; row < screenPoints.GetLength(0); row++) {
            ...
            for (int col = 0; col < screenPoints.GetLength(1); col++)
            {
                ...
                if (hitLaserable)
                {
                    ...
                    else if (rayCastsHits[row, col].collider.gameObject.CompareTag("InterestingObject")){
